...
function call_remain_time($race_kind, $place, $place2, $scrpt_str, $jp) {
...
if ( $jp=='1') {
	   $sql = "select * from jp_raceinput where race_date ='$race_date' and race_kind='$race_kind' limit 1";
	   $scrpt_str = "jp".$scrpt_str;
	   //echo $sql."<br>";
} else {  
...
//		$timenow = date ("YmdHis");
 ...
 $str = " 
  ...
  <tr>
	...
	<table width='165' border='0' cellpadding='0' cellspacing='0'>
	<tr>
		<td class='txt_left' align=center>- ".$place." ".$i."경주 -</td></tr>";
 } else  {
  ...
  <tr>
	...
	<table width='165' border='0' cellpadding='0' cellspacing='0'>
	<tr>
		<td align=left>-<font size=1>일본경마</font>(".$place2.") ".$i."경주 - </td></tr>";
...
<script language=javascript src="/js/jp_remain_time.js"></script>
...
echo "function remain_time_jp$i(){";

echo "
    //var remain_times = \"마감 \";
	var remain_times_jp$i = \"\";
	remain_seconds_jp$i = curRemainTime_jp$i; 

   if ( (remain_seconds_jp$i/86400) >= 1)  
	{	remain_times_jp$i = remain_times_jp$i + Math.floor(remain_seconds_jp$i/86400) + \"일\";	}
	if ( (remain_seconds_jp$i%86400)/3600 >= 1)  
	{	remain_times_jp$i = remain_times_jp$i + Math.floor((remain_seconds_jp$i%86400)/3600) + \"시간\";	}
	if ( (remain_seconds_jp$i%3600)/60 >= 1)  
	{	remain_times_jp$i = remain_times_jp$i + Math.floor((remain_seconds_jp$i%3600)/60) + \"분\";	}

	if ( ((remain_seconds_jp$i%60)/60 >=0) && (remain_seconds_jp$i != \"\") )  
	{	remain_times_jp$i = remain_times_jp$i + Math.floor(remain_seconds_jp$i%60) + \"초\";	}
	else	{		remain_times_jp$i = \"마감\";	}

	curRemainTime_jp$i--;  // 남은 시각
	passTime_jp$i++;		  // 현재페이지를 연상태에서 지나간 시간
	tmp = document.getElementById('remain_times_jp$i');
	tmp.innerHTML = \"<font color=red>\" + remain_times_jp$i + \"</font>\";
	setTimeout(\"remain_time_jp$i()\",1000);
...
$jp ='0';
echo call_remain_time('t01', $t01_str, $t01_str2, $t01_scrpt_str, $jp);
echo call_remain_time('h01', $h01_str, $h01_str2, $h01_scrpt_str, $jp);	
echo call_remain_time('h02', $h02_str, $h02_str2, $h02_scrpt_str, $jp);	
echo call_remain_time('h03', $h03_str, $h03_str2, $h03_scrpt_str, $jp);
echo call_remain_time('k01', $k01_str, $k01_str2, $k01_scrpt_str, $jp);	
echo call_remain_time('k02', $k02_str, $k02_str2, $k02_scrpt_str, $jp);	
echo call_remain_time('k03', $k03_str, $k03_str2, $k03_scrpt_str, $jp);	
...
if($rowp) {
	...
	for ($i=0;$i<$noplace-1;$i++) {
		...
		//echo "$i, $place";
		$jp ='1';
		echo call_remain_time($placearray[$i], $placecommon, $place, $placearray[$i], $jp);
