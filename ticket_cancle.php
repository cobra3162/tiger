...
for ($i=0; $i<count($chk); $i++) 
{
    ...
    $start_time = $_POST['startTime'][$k];

   	//echo $start_time." / ".$timenow." / ".$sql;
   	//	exit;
         
	if ($start_time < $timenow)  {
		echo "<script> alert('마감시간경과로 구매취소가 불가능합니다');</script>";
		 continue;  // 시간경과 또는 이미취소된 티켓
	} 
	...
	*/
    ...
    $sql6 = "select * from account where ticket_no = '{$_POST['no'][$k]}' and kind!='cancle' and id='$id' order by no desc limit 1";
	echo $sql6;
	$row6 = sql_fetch($sql6);
	$amount = $row6[amount];
	$event = $row6[event];
